...
public class Testing123 {
					...
					if(easy) {
						if(easyGame.includesChar(button.getText())) {
							...
							if(easyGame.checkGuess()) {
								JOptionPane.showConfirmDialog(null, "<html>The man has not been hung:)<br/>You have successfully completed this round!<br/>Congrats, good luck on the next question!</html>", "Success", JOptionPane.DEFAULT_OPTION);
								nextQuestion();		
							}	
						}
						else {
							...
							if(easyGame.getLives() <= 0) {
								JOptionPane.showConfirmDialog(null, "<html>The man has been hung:(<br/>You have failed this round!<br/>However, you may continue to the next round...</html>", "Game Over", JOptionPane.DEFAULT_OPTION);							
								nextQuestion();
							}
						}
					}
	...
	class DiffPanel extends JPanel implements ActionListener {
		...
		DiffPanel() {
			...
			normalBut = new JButton("Medium (3 Lives)");
